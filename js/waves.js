...
function gameloop(delta) {
    ...
    // Waves created by the user should expand and be removed once they reach
    // maximum size. They don't have to be replaced.
    for (let wave of userCreatedWaves) {
        if (wave.radius < wave.max_radius) {
            wave.expand();
        } else {
            for (let gc of wave.getGraphicalComponents()) {
...
function handleClick(e) {
    let wave = new Wave(e.pageX, e.pageY);
    userCreatedWaves.add(wave);
    for (let gc of wave.getGraphicalComponents()) {
        app.stage.addChild(gc);
    }
}
